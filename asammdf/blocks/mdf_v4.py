    ...
    * ``param`` - row size used for tranposizition, in case of tranposed zipped blockss
    ...
    def _read(self):
        ...
        if raw_can:
            ...
            for sigs, message_id, message_name, cg_source, can_id in processed_can:
                ...
                group["data_group"].comment = f'From message {hex(message_id)}="{message_name}"'
    ...
    def _get_not_byte_aligned_data(self, data, group, ch_nr):
        ...
        if dependencies and isinstance(dependencies[0], ChannelArrayBlock):
            ...
            size = bit_count >> 3
            shape = tuple(
                ca_block[f"dim_size_{i}"] for i in range(ca_block["dims"])
            )
                    ...
                    if parent is not None:
                        ...
                        if kind_ == "b":
                            ...
                            if dtype_.byteorder == '>':
                                ...
                                if bit_offset:
                                    if kind_ == "i":
                                        vals = vals.astype(dtype(f"{dtype_.byteorder}u{size}"))
                                        vals >>= bit_offset
                                    else:
                    ...
                    if signal_data:
                        ...
                        if data_type == v4c.DATA_TYPE_BYTEARRAY:
                            ...
                            if data_type == v4c.DATA_TYPE_STRING_UTF_16_BE:
                                ...
                                raise MdfException(f'wrong data type "{data_type}" for vlsd channel')
